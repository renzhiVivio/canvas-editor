...
import {WorkareaLayout, WorkareaObject} from '../helper';
...
class WorkareaHandler {
    ...
    public initialize() {
        let {workareaOption} = this.handler;
        if (!workareaOption || workareaOption.width === 0 || workareaOption.height === 0) {
            if (!workareaOption) {
                workareaOption = defaults.workareaOption;
            }

            workareaOption.width = this.handler.canvas.width;
            workareaOption.height = this.handler.canvas.height;
        }
        ...
        this.handler.workarea.workareaHeight = workareaOption.height;
        this.handler.workarea.workareaWidth = workareaOption.width;
