...
import {gridOption} from "../defaults";
...
class GridHandler {
     ...
     */
    public initialize = () => {
        const {grid, lineColor, borderColor, enabled} = this.handler.gridOption ?? gridOption;
        if (enabled && grid) {
            const width = 5000;
            ...
            if (target.type === 'activeSelection') {
                ...
                activeSelection.getObjects().forEach((obj: any) => {
                    if (obj.superType === 'node') {
                        const left = target.left + obj.left + (target.width ?? 0) / 2;
                        const top = target.top + obj.top + (target.height ?? 0) / 2;
