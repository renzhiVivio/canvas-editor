 ...
 */
class LinkHandler {
    private port: PortObject | null = null;
    handler: Handler;
    ...
    generate = (port: PortObject) => {
        if (!port) {
            console.error('Does not exist target port.');
            return;
        }
    ...
    create = (link: LinkOption, loaded = false) => {
        ...
        this.handler.canvas.renderAll();
        if (fromNode && toNode && createdObj.setPort && fromPort && toPort) {
    ...
    removeFrom = (link: LinkObject) => {
        if (link.fromPort) {
            ...
            if (link.setPortEnabled && link.fromNode) {
    ...
    isConnected = (port: PortObject) => {
        console.error(port.enabled, 'A connected node already exists.');
        return !port.enabled;
    };
    ...
    isSameNode = (port: PortObject) => {
        ...
        console.error(!validate, 'Cannot select the same node.');
        return validate;
    };
    ...
    isDuplicate = (port: PortObject) => {
        ...
        console.error(!validate, 'Duplicate connections cannot be made.');
        return validate;
    };
    ...
    isDrawing = () => {
        ...
        console.error(!validate, 'Already drawing links.');
        return validate;
    };
}
