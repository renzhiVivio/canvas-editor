...
class PortHandler {
    ...
    create = (target: NodeObject) => {
        ...
        const toPort = target.createToPort((target.left ?? 0) + (target.width ?? 0) / 2, (target.top ?? 0));
        if (toPort) {
            toPort.on('mouseover', () => {
                if (
        ...
        const fromPort = target.createFromPort?.((target.left ?? 0) + (target.width ?? 0) / 2, (target.top ?? 0) + (target.height ?? 0));
    ...
    setCoords = (target: NodeObject) => {
        if (target.toPort) {
            const toCoords = {
                left: (target.left ?? 0) + (target.width ?? 0) / 2,
                top: target.top,
            };
            target.toPort.set({
                ...toCoords,
            ...
            if (target.toPort.links?.length) {
                target.toPort.links?.forEach(link => {
                    ...
                    if (fromPort) {
                        this.handler.linkHandler.setCoords((fromPort.left ?? 0), (fromPort.top ?? 0), toCoords.left, (toCoords.top ?? 0), link);
                    }
                });
            }
        }
        if (target.fromPort) {
            const fromCoords = {
                left: (target.left ?? 0) + (target.width ?? 0) / 2,
                top: (target.top ?? 0) + (target.height ?? 0),
            };
                ...
                if (port.links?.length) {
                    port.links?.forEach(link => {
                        this.handler.linkHandler.setCoords(
                            left,
                            top,
                            (link.toNode?.toPort?.left ?? 0),
                            (link.toNode?.toPort?.top ?? 0),
                            link,
                        );
                    });
                }
