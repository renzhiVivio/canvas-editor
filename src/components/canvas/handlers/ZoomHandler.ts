...
import {FabricObject} from "../objects/helper";
...
class ZoomHandler {
    ...
    /**
     * Zoom to fit workarea
     *
     */
    public zoomToFit = () => {
        if (this.handler.workarea == null)
            return;
        ...
        if (this.handler.workarea.height! >= this.handler.workarea.width!) {
            scaleX = scaleY;
            if (this.handler.canvas.getWidth() < (this.handler.workarea?.width ?? 0) * scaleX) {
                scaleX = scaleX * (this.handler.canvas.getWidth() / ((this.handler.workarea?.width ?? 0) * scaleX));
            }
        } else {
            if (this.handler.canvas.getHeight() < (this.handler.workarea?.height ?? 0) * scaleX) {
                scaleX = scaleX * (this.handler.canvas.getHeight() / ((this.handler.workarea?.height ?? 0) * scaleX));
            }
        }
