...
import {FabricEvent, FabricObject} from '../objects/helper';
...
class EventHandler {
    ...
    public initialize() {
        if (this.handler.editable) {
            ...
            this.handler.canvas.on({
                ...
                'object:scaled': this.scaled,       // deprecated in fabricjs 5.0
                'object:moving': this.moving,
                'object:moved': this.moved,         // deprecated in fabricjs 5.0
                'object:rotating': this.rotating,
                'object:rotated': this.rotated,     // deprecated in fabricjs 5.0
                ...
                'text:editing:exited': this.textEditingExited,
            });
        } else {
            // @ts-ignore
            this.handler.canvas.on({
    ...
    public destroy = () => {
        if (this.handler.editable) {
            ...
            this.handler.getCanvasObjects().forEach(object => {
    ...
    public modified = (opt: FabricEvent) => {
        ...
        // @ts-ignore
        const actionType = opt.action;

        if (actionType === 'drag') {
            this.moved(opt);
        } else if (actionType === 'rotate') {
            this.rotated(opt);
        } else if (actionType === 'scale' || actionType === 'scaleX' || actionType === 'scaleY') {
            this.scaled(opt);
        } else if (actionType === 'resizing' && target.type === 'textbox') {
            // 注意textbox的scaleX会被视为resizing
            this.scaled(opt);
        }
    ...
    public moving = (opt: FabricEvent) => {
        ...
        if (this.handler.interactionMode === 'crop') {
            ...
            // let data=[] as any[];
            // data = this.handler.exportJSON().filter(obj => {
            //     // 注意网格不需要渲染
            //     if (!obj.id || obj.id==='grid') {
            //         return false;
            //     }
            //     return true;
            // });
            // 设置object移动边界，注意画布实际高度要比defaults.canvasOption.height少20px
            // 所以这里要多减去20
            // 这几行是控制组件拖动不要超出workarea的，暂时不需要
            // target.setCoords();
            // if(target.top<0)target.top = 0;
            // if(target.left<0)target.left = 0;
            // if (target.top>defaults.canvasOption.height-20-target.height*target.scaleY)target.top=defaults.canvasOption.height-20-target.height*target.scaleY;
            // if (target.left>defaults.canvasOption.width-target.width*target.scaleX)target.left=defaults.canvasOption.width-target.width*target.scaleX;
     ...
     * @param {FabricEvent} opt
     */
    public textEditingExited = (opt: FabricEvent) => {
        if (this.handler.interactionMode !== 'crop') {
            this.handler.transactionHandler.save('textExited');
        }
    }
    ...
    public mousemove = (opt: FabricEvent) => {
        const event = opt as FabricEvent<MouseEvent>;
        // console.log(`mouse moving on target: ${event.target}, ${event.target?.superType}`);
        ...
        if (this.handler.interactionMode === 'polygon') {
            ...
            if (this.handler.activeLine && this.handler.activeLine.class === 'line') {
                ...
                points[this.handler.pointArray?.length ?? 0] = {
                    x: pointer.x,
                    y: pointer.y,
                };
    ...
    public mouseup = (opt: FabricEvent) => {
        ...
        if (this.handler.editable && this.handler.guidelineOption?.enabled) {
            ...
            if (this.handler.guidelineHandler.verticalLines) {
                this.handler.guidelineHandler.verticalLines.length = 0;
            }

            if (this.handler.guidelineHandler.horizontalLines) {
    ...
    public selection = (opt: FabricEvent) => {
        ...
        if (onSelect) {
            // don't know why, but the target is always undefined, debug and find opt.selected is actual selected objects
            // @ts-ignore
            onSelect(opt.selected);
        }
    };
    ...
    public resize = (nextWidth: number, nextHeight: number) => {
        ...
        // 注意计算这里用！可能会得出NaN,所以NaN时取0
        const diffWidth = nextWidth / 2 - this.handler.width! / 2 || 0;
        const diffHeight = nextHeight / 2 - this.handler.height! / 2 || 0;
        ...
        if (this.handler.workarea.layout === 'fixed') {
            ...
            // 原来的react-design-editor有网格的时候不渲染workarea，所以我们comment掉
            // if (this.handler.gridOption?.enabled) {
            //     return;
            // }
            let data = this.handler.exportJSON().filter(obj => {
                // 注意网格不需要渲染
                if (!obj.id || obj.id === 'grid') {
                    return false;
                }
            ...
            this.handler.canvas.getObjects().forEach((obj: FabricObject) => {
                if (obj.id !== 'workarea' && obj.id !== 'grid') {
    ...
    public keydown = (e: KeyboardEvent) => {
        ...
        if (!keyEvent || !Object.keys(keyEvent).length) {
            return;
        }
